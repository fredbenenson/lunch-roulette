      ...
      @results.map do |type, result|
        result.map.with_index do |set, set_index|
          ...
          CSV.open(file, "w") do |csv|
            puts "#{type.capitalize} Set Candidate: #{set_index + 1}" if config.options[:verbose_output]
            ...
            set.groups.each.with_index do |group, group_index|
              ...
              o = "Group #{group_index + 1}: #{group.inspect}\n\tAverage Score: #{group.average_score.round(4)}\n\tScore Breakdown:#{group.scores}\n\n"
              puts o if config.options[:verbose_output]
              csv << [group.average_score, *s, group.inspect].flatten
            end
    ...
    def get_new_staff_csv(staff)
      ...
      CSV.open(file, "w") do |csv|
        ...
        staff.each do |luncher|
          o = [ luncher.user_id, luncher.name, luncher.start_date, luncher.floor, luncher.team, luncher.specialty, [luncher.previous_lunches, person_lunch_mappings[luncher.user_id]].flatten.join(",") ]
